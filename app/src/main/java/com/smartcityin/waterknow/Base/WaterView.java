...
public class WaterView extends View {
    ...
    public WaterView(Context context, AttributeSet attrs) {
        ...
        textPaint = new Paint();
        textPaint.setARGB(255,102,153,255);
    ...
    public void setWaterBalance(int balance){
    ...
    private void drawText(Canvas canvas) {
        ...
        // 画分数
            canvas.drawText("" +score+"m³", radius, radius + clipRadius/2, textPaint);


        // 画固定值分
        textPaint.setTextSize(clipRadius/4);
        // 画固定值立即优化
        canvas.drawText("用水额度", radius, radius, textPaint);
    ...
    private void drawLine(final Canvas canvas) {
        ...
        for (int i = 0; i <= 100; i++) {
            ...
            if (c <= targetAngle && targetAngle != 0) {// 如果累计画过的角度，小于当前有效刻度
                ...
                //调整水的占用量的颜色
                linePaint.setARGB(255, 2, 211, 253);
